...
- Downloads preserve the uploaded format: PDF uploads return the original PDF layout, text uploads export TXT.
...
No backend is required. PDF parsing and icons load from CDNs, so the deployed app needs internet access.
...
For PDF uploads, the download keeps the exact uploaded PDF layout. The generated draft remains available for copy/editing because a flattened PDF cannot be reliably rewritten into the same visual template in the browser without the original source document.
